...
* 「管理 ATC5碼 顏色對照」功能：
  - 可自訂要以特定顏色標示的 ATC5 碼
  - 提供 ATC5 碼查詢連結: https://atcddd.fhi.no/atc_ddd_index/
...
![image](https://github.com/leescot/img/blob/main/NHI_analyzer_setting_09.jpg)
...
* 表格進階自訂功能：
  - 優先顯示項目（最多20項）
    - 讓使用者設定自己常常需要先看的檢驗資料項目於表格的上方
    - 檢驗醫令代碼查詢: https://info.nhi.gov.tw/INAE5000/INAE5001S01
    - 檢驗項目統一顯示名稱
      - 不同院所同一種醫令代碼上傳的檢驗名稱會是不同的。舉例醫令代碼 09025C，可能會有以下名稱: GOT, AST(GOT), 麩胺酸苯醋酸轉氨基酶。此時若設定 09025C 為「GOT」，不管院所上傳的檢驗項目為何，都統一顯示為使用者自訂的「GOT」
      - 此功能僅適用於單一醫令代碼對應單一檢驗項目的情況，不適用於如 08011C(全套血液檢查，包了 Hb, MCV, WBC等)、 09015C(肌酸肝，通常包含 Cr, eGFR 兩項內容) 這類包含多個項目的醫令代碼。
    - CBC/DC、UA 項目顯示設定
      - CBC/DC、UA 因為檢驗項目很多，會造成表格的顯示長度變的很長。此處可讓使用者設定: 全部顯示、不顯示、顯示部份。
      - 設定「顯示部份」，可設定關鍵字。例如 UA 處設定 pro, RBC, WBC，則 UA 檢驗項目名稱有包含以上關鍵字的才會顯示在表格中。
  - 若有設定「異常檢驗值變色顯示」，表格中的數值也會依參考值來變色。
  - 若有設定「常見檢驗項目縮寫」，於表格中也會啟動此功能。
    - 建階用法: 使用於包含多個項目的醫令代碼的項目精簡法。
    - 舉例1: 08011C(全套血液檢查) 的血紅素，有些院所上傳為 Hb，有些院所上傳為 Hgb。可在「常見檢驗項目縮寫」設定 Hgb 顯示為 Hb，就可將兩邊來源數據在表格中合而為一。
    - 舉例2: 09015C(肌酸肝，通常包含 Cr, eGFR 兩項內容)，每家院所上傳為腎絲球過濾率的文字都不同，可將自己常見到的相關文字統一在「常見檢驗項目縮寫」設定為"eCCr"，就可來在表格中合而為一。

![image](https://github.com/leescot/img/blob/main/NHI_analyzer_setting_10.jpg)
